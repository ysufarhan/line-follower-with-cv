...
def setup_fuzzy_logic():
    ...
    error['Z']  = fuzz.trimf(error.universe, [-15, 0, 15])  # Diperlebar
    ...
    output['Z']  = fuzz.trimf(output.universe, [-15, 0, 15])  # Lebih lebar
...
def calculate_motor_pwm(kontrol, base_pwm=50, scaling_factor=0.20):
    if abs(kontrol) < 8:
        kontrol_scaled = 0  # Jalankan dengan PWM simetris
    else:
...
def main():
    ...
    try:
        while True:
            ...
            cv2.line(frame_with_line, (160, 120), (160, 240), (0, 255, 0), 2)
            if line_detected:
