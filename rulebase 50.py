...
def setup_fuzzy_logic():
    ...
    # Rule Base
   rules = [
        # --- Bagian 1: Error Negative Large (NL) --- (5 Aturan)
        ctrl.Rule(error['NL'] & delta['NL'], output['L']),   # Sangat jauh kiri & menjauh cepat -> Belok keras kiri
        ctrl.Rule(error['NL'] & delta['NS'], output['L']),   # Sangat jauh kiri & pelan menjauh -> Belok keras kiri
        ctrl.Rule(error['NL'] & delta['Z'], output['LM']),   # Sangat jauh kiri & stabil -> Belok kuat kiri
        ctrl.Rule(error['NL'] & delta['PS'], output['LM']),  # Sangat jauh kiri & mendekat -> Tetap belok kuat kiri (proaktif)
        ctrl.Rule(error['NL'] & delta['PL'], output['LS']),  # Sangat jauh kiri & cepat mendekat -> Belok sedang kiri (mencegah overshoot)

        # --- Bagian 2: Error Negative Medium (NM) --- (5 Aturan)
        ctrl.Rule(error['NM'] & delta['NL'], output['LM']),  # Cukup jauh kiri & menjauh cepat -> Belok kuat kiri
        ctrl.Rule(error['NM'] & delta['NS'], output['LM']),  # Cukup jauh kiri & pelan menjauh -> Belok kuat kiri
        ctrl.Rule(error['NM'] & delta['Z'], output['LS']),   # Cukup jauh kiri & stabil -> Belok sedang kiri
        ctrl.Rule(error['NM'] & delta['PS'], output['LS']),  # Cukup jauh kiri & mendekat -> Belok sedang kiri
        ctrl.Rule(error['NM'] & delta['PL'], output['Z']),   # Cukup jauh kiri & cepat mendekat -> Luruskan

        # --- Bagian 3: Error Negative Small (NS) --- (5 Aturan)
        ctrl.Rule(error['NS'] & delta['NL'], output['LS']),  # Sedikit kiri & menjauh cepat -> Belok sedang kiri
        ctrl.Rule(error['NS'] & delta['NS'], output['LS']),  # Sedikit kiri & pelan menjauh -> Belok sedang kiri
        ctrl.Rule(error['NS'] & delta['Z'], output['Z']),    # Sedikit kiri & stabil -> Luruskan
        ...
        # --- Bagian 4: Error Zero (Z) --- (5 Aturan)
        ctrl.Rule(error['Z'] & delta['NL'], output['Z']),    # Di tengah & menjauh cepat -> Luruskan (tetap stabil)
        ctrl.Rule(error['Z'] & delta['NS'], output['Z']),    # Di tengah & pelan menjauh -> Luruskan
        ctrl.Rule(error['Z'] & delta['Z'], output['Z']),     # Di tengah & stabil -> Tetap lurus (Prioritas utama)
        ctrl.Rule(error['Z'] & delta['PS'], output['Z']),    # Di tengah & mendekat -> Luruskan
        ctrl.Rule(error['Z'] & delta['PL'], output['Z']),    # Di tengah & cepat mendekat -> Luruskan

        # --- Bagian 5: Error Positive Small (PS) --- (5 Aturan)
        ...
        ctrl.Rule(error['PS'] & delta['Z'], output['Z']),    # Sedikit kanan & stabil -> Luruskan
        ctrl.Rule(error['PS'] & delta['PS'], output['RS']),  # Sedikit kanan & pelan menjauh -> Belok sedang kanan
        ctrl.Rule(error['PS'] & delta['PL'], output['RS']),  # Sedikit kanan & menjauh cepat -> Belok sedang kanan

        # --- Bagian 6: Error Positive Medium (PM) --- (5 Aturan)
        ctrl.Rule(error['PM'] & delta['NL'], output['Z']),   # Cukup jauh kanan & cepat mendekat -> Luruskan
        ctrl.Rule(error['PM'] & delta['NS'], output['RS']),  # Cukup jauh kanan & mendekat -> Belok sedang kanan
        ctrl.Rule(error['PM'] & delta['Z'], output['RM']),   # Cukup jauh kanan & stabil -> Belok kuat kanan
        ctrl.Rule(error['PM'] & delta['PS'], output['RM']),  # Cukup jauh kanan & pelan menjauh -> Belok kuat kanan
        ctrl.Rule(error['PM'] & delta['PL'], output['RM']),  # Cukup jauh kanan & menjauh -> Belok kuat kanan

        # --- Bagian 7: Error Positive Large (PL) --- (5 Aturan)
        ctrl.Rule(error['PL'] & delta['NL'], output['LS']),  # Sangat jauh kanan & cepat mendekat -> Belok sedang kanan (mencegah overshoot)
        ctrl.Rule(error['PL'] & delta['NS'], output['RM']),  # Sangat jauh kanan & mendekat -> Tetap belok kuat kanan (proaktif)
        ctrl.Rule(error['PL'] & delta['Z'], output['RM']),   # Sangat jauh kanan & stabil -> Belok kuat kanan
        ctrl.Rule(error['PL'] & delta['PS'], output['R']),   # Sangat jauh kanan & pelan menjauh -> Belok keras kanan
        ctrl.Rule(error['PL'] & delta['PL'], output['R']),   # Sangat jauh kanan & menjauh cepat -> Belok keras kanan

        # --- Bagian 8: Aturan Tambahan untuk Memperkuat Respons dan Transisi (15 Aturan) ---

        # Dorong Kembali ke Tengah Lebih Cepat (ketika error besar tapi delta mendekat cepat)
        ctrl.Rule(error['NL'] & delta['PL'], output['Z']),   # Jika sangat jauh kiri tapi mendekat sangat cepat, luruskan untuk mencegah overshoot.
        ctrl.Rule(error['PL'] & delta['NL'], output['Z']),   # Jika sangat jauh kanan tapi mendekat sangat cepat, luruskan.

        # Memperkuat respons belok saat transisi dari 'Z' ke 'NS'/'PS'
        ctrl.Rule(error['Z'] & delta['NS'], output['RS']),  # Jika di tengah tapi mulai bergeser ke kiri, dorong sedikit ke kanan
        ctrl.Rule(error['Z'] & delta['PS'], output['LS']),  # Jika di tengah tapi mulai bergeser ke kanan, dorong sedikit ke kiri

        # Menjaga kecepatan di tikungan sedang (error Medium)
        ctrl.Rule(error['NM'] & delta['PS'], output['LS']),
        ctrl.Rule(error['PM'] & delta['NS'], output['RS']),

        # Respons darurat jika sudah sangat dekat dengan batas frame
        # Ini mengasumsikan error mendekati +/-250
        ctrl.Rule(error['NL'] & delta['PS'], output['LM']), # Meskipun mendekat, jika masih sangat jauh, tetap dorong kuat.
        ctrl.Rule(error['PL'] & delta['NS'], output['RM']),

        # Meningkatkan stabilitas saat error kecil dan delta sangat cepat (kemungkinan overshoot)
        ctrl.Rule(error['NS'] & delta['PL'], output['Z']),
        ctrl.Rule(error['PS'] & delta['NL'], output['Z']),

        # Aturan untuk memastikan dorongan yang cukup di awal tikungan
        ctrl.Rule(error['NM'] & delta['Z'], output['LS']),
        ctrl.Rule(error['PM'] & delta['Z'], output['RS']),

        # Aturan tambahan untuk memperkuat belokan ekstrem
        ctrl.Rule(error['NL'] & delta['Z'], output['LM']),  # Ketika error NL dan stabil, lebih agresif dari sebelumnya
        ctrl.Rule(error['PL'] & delta['Z'], output['RM']),  # Ketika error PL dan stabil, lebih agresif dari sebelumnya

        # Kondisi 'Z' dengan delta ekstrem (ini bisa menunjukkan robot mulai melenceng)
        ctrl.Rule(error['Z'] & delta['NL'], output['LS']), # Jika di tengah tapi ada perubahan delta cepat ke kiri, dorong sedikit ke kanan
        ctrl.Rule(error['Z'] & delta['PL'], output['RS']), # Jika di tengah tapi ada perubahan delta cepat ke kanan, dorong sedikit ke kiri

        # Penyesuaian untuk menjaga posisi di tengah (kalau ada osilasi)
        ctrl.Rule(error['NS'] & delta['PL'], output['Z']),
        ctrl.Rule(error['PS'] & delta['NL'], output['Z']), 
    ]
