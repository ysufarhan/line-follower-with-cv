...
class LineRecovery:
    def __init__(self):
        ...
        self.search_speed = 35 # Kecepatan PWM saat berputar mencari garis (DIUBAH: Disesuaikan)

    def handle_line_lost(self, ser_instance):
        self.lost_count += 1
        
        # Tahap 1: Diam sebentar
        if self.lost_count < 10: # Diam selama 10 frame (DIUBAH: Durasi sedikit dipersingkat)
...
def process_image(frame, display_mode=False):
    ...
    # DIUBAH: ROI dimulai lebih tinggi (melihat lebih jauh ke depan)
    roi_start_y = 120 # DIUBAH: Dulu 160, sekarang 120 (melihat lebih jauh ke depan)
    ...
    if frame.shape[0] < roi_end_y:
        ...
        return None, None, None, 0 # Menambahkan nilai default untuk roi_start_y
...
def calculate_motor_pwm(kontrol, base_pwm=35, scaling_factor=0.08): 
    FLC_DEAD_ZONE = 10 
    ...
    try:
        while True:
            ...
            if line_detected:
                ...
                if frame_count % 10 == 0:
                    print(f"[DEBUG] Error: {error:4d}, Delta: {delta_error:3d}, FLC:{kontrol:6.2f}, PWM: L{pwm_kiri} R{pwm_kanan}") # DIUBAH: Format Delta
            else:
                recovery_action = line_recovery_handler.handle_line_lost(ser)
                prev_error = 0 # Reset error untuk mencegah lonjakan besar saat garis ditemukan
                if frame_count % 20 == 0:
            ...
            if DISPLAY_GUI:
                ...
                if line_detected:
                    ...
                    # HATI-HATI: Memanggil handle_line_lost() di sini akan memanggilnya dua kali per frame saat garis hilang.
                    # Ini bisa mempercepat lost_count secara tidak akurat. Lebih baik tampilkan saja state yang sudah ada.
                    # cv2.putText(frame_for_display, f"ACTION: {line_recovery_handler.handle_line_lost(ser)}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
