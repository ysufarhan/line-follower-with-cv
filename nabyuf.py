...
def setup_fuzzy_logic():
    # Menggunakan rentang yang lebih luas dan simetris untuk error dan delta
    error = ctrl.Antecedent(np.arange(-320, 321, 1), 'error') # Maks error sekitar +-320 untuk frame 640
    delta = ctrl.Antecedent(np.arange(-100, 101, 1), 'delta') # Delta error umumnya tidak sebesar error absolut
    output = ctrl.Consequent(np.arange(-100, 101, 1), 'output')

    # Membership functions untuk ERROR
    # Membuat NL/PL lebih "merata" dan Z sedikit lebih lebar untuk stabilitas di tengah
    error['NL'] = fuzz.trimf(error.universe, [-320, -200, -60]) # Mencakup error besar
    error['NS'] = fuzz.trimf(error.universe, [-70, -25, -5])   # Respons awal untuk deviasi kecil
    error['Z']  = fuzz.trimf(error.universe, [-10, 0, 10])     # Zona stabil di tengah
    error['PS'] = fuzz.trimf(error.universe, [5, 25, 70])      # Respons awal untuk deviasi kecil
    error['PL'] = fuzz.trimf(error.universe, [60, 200, 320])   # Mencakup error besar

    # Membership functions untuk DELTA ERROR
    # Penting untuk mendeteksi perubahan cepat
    delta['NL'] = fuzz.trimf(delta.universe, [-100, -60, -20]) # Perubahan negatif besar (garis bergerak cepat ke kanan)
    delta['NS'] = fuzz.trimf(delta.universe, [-25, -10, -1])  # Perubahan negatif kecil
    delta['Z']  = fuzz.trimf(delta.universe, [-3, 0, 3])      # Tidak ada perubahan signifikan
    delta['PS'] = fuzz.trimf(delta.universe, [1, 10, 25])     # Perubahan positif kecil
    delta['PL'] = fuzz.trimf(delta.universe, [20, 60, 100])   # Perubahan positif besar (garis bergerak cepat ke kiri)

    # Membership functions untuk OUTPUT (Kontrol PWM)
    # Membuat LS/RS sedikit lebih agresif
    output['L']  = fuzz.trimf(output.universe, [-100, -75, -30]) # Belok kiri keras
    output['LS'] = fuzz.trimf(output.universe, [-40, -15, -5])  # Belok kiri sedang/ringan
    output['Z']  = fuzz.trimf(output.universe, [-5, 0, 5])      # Lurus
    output['RS'] = fuzz.trimf(output.universe, [5, 15, 40])    # Belok kanan sedang/ringan
    output['R']  = fuzz.trimf(output.universe, [30, 75, 100])  # Belok kanan keras

    # PERUBAHAN KRITIS: Aturan Fuzzy
    # Fokus pada antisipasi dan koreksi kuat
    rules = [
        # Ketika error sangat besar (jauh dari tengah)
        ctrl.Rule(error['NL'] & delta['NL'], output['L']), # Jauh kanan, makin menjauh -> Keras ke kiri
        ctrl.Rule(error['NL'] & delta['NS'], output['L']), # Jauh kanan, masih menjauh -> Keras ke kiri
        ctrl.Rule(error['NL'] & delta['Z'], output['LS']), # Jauh kanan, tetap di situ -> Sedang ke kiri
        ctrl.Rule(error['NL'] & delta['PS'], output['LS']),# Jauh kanan, mulai mendekat -> Sedang ke kiri (agresif)
        ctrl.Rule(error['NL'] & delta['PL'], output['Z']), # Jauh kanan, cepat sekali mendekat -> Luruskan (hampir kembali)

        # Ketika error cukup kecil tapi negatif (sedikit di kanan)
        ctrl.Rule(error['NS'] & delta['NL'], output['LS']),# Sedikit kanan, makin menjauh -> Lebih keras ke kiri
        ctrl.Rule(error['NS'] & delta['NS'], output['LS']),# Sedikit kanan, menjauh -> Lebih keras ke kiri
        ctrl.Rule(error['NS'] & delta['Z'], output['Z']),  # Sedikit kanan, stabil -> Luruskan
        ctrl.Rule(error['NS'] & delta['PS'], output['Z']), # Sedikit kanan, mendekat -> Luruskan
        ctrl.Rule(error['NS'] & delta['PL'], output['RS']),# Sedikit kanan, cepat sekali mendekat -> Balik kanan sedikit (over-correct)

        # Ketika error nol (di tengah)
        ctrl.Rule(error['Z'] & delta['NL'], output['LS']), # Tengah, tapi menjauh ke kanan -> Koreksi kiri sedikit
        ctrl.Rule(error['Z'] & delta['NS'], output['Z']),  # Tengah, hampir menjauh ke kanan -> Luruskan
        ctrl.Rule(error['Z'] & delta['Z'], output['Z']),   # Tengah, stabil -> Luruskan
        ctrl.Rule(error['Z'] & delta['PS'], output['Z']),  # Tengah, hampir menjauh ke kiri -> Luruskan
        ctrl.Rule(error['Z'] & delta['PL'], output['RS']), # Tengah, tapi menjauh ke kiri -> Koreksi kanan sedikit

        # Ketika error cukup kecil tapi positif (sedikit di kiri)
        ctrl.Rule(error['PS'] & delta['NL'], output['LS']),# Sedikit kiri, cepat sekali mendekat -> Balik kiri sedikit (over-correct)
        ctrl.Rule(error['PS'] & delta['NS'], output['Z']), # Sedikit kiri, mendekat -> Luruskan
        ctrl.Rule(error['PS'] & delta['Z'], output['Z']),  # Sedikit kiri, stabil -> Luruskan
        ctrl.Rule(error['PS'] & delta['PS'], output['RS']),# Sedikit kiri, menjauh -> Lebih keras ke kanan
        ctrl.Rule(error['PS'] & delta['PL'], output['RS']),# Sedikit kiri, makin menjauh -> Lebih keras ke kanan

        # Ketika error sangat besar (jauh dari tengah)
        ctrl.Rule(error['PL'] & delta['NL'], output['Z']), # Jauh kiri, cepat sekali mendekat -> Luruskan (hampir kembali)
        ctrl.Rule(error['PL'] & delta['NS'], output['RS']),# Jauh kiri, mulai mendekat -> Sedang ke kanan (agresif)
        ctrl.Rule(error['PL'] & delta['Z'], output['RS']), # Jauh kiri, tetap di situ -> Sedang ke kanan
        ctrl.Rule(error['PL'] & delta['PS'], output['R']), # Jauh kiri, masih menjauh -> Keras ke kanan
        ctrl.Rule(error['PL'] & delta['PL'], output['R']), # Jauh kiri, makin menjauh -> Keras ke kanan
    ]
